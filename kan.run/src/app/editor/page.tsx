...
export default function EditorPage() {
  ...
  // ノートの編集画面に遷移
  const goToEditNote = useCallback((noteId: string) => {
      ...
      {notes?.map((note) => (
        ...
        >
          ...
          <div className="text-xs text-gray-500">
            <span>更新: {new Date(note.lastChangedAt).toLocaleDateString('ja-JP')}</span>
          </div>
        </div>
      ))}
    </div>
      ...
      <div className="bg-white rounded-lg shadow">
        ...
        <div className="p-4">
          {(!notes || notes.length === 0) && !isLoading ? (
            ...
            viewMode === 'grid' ? renderNotesGrid() : renderNotesList()
          )}
